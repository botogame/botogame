...
На заре цифровой эры, когда первые байты падали на землю подобно дождю, появился Путь Программиста. Подобно мастерам боевых духов, каждый начинающий кодер должен был выбрать свой путь: слабые гибли в пустынных скриптах, другие навеки застревали в проклятом Notepad 6.66, где строки ломали разум, а отступы рвали душу.

Но истинные воины кода знали: каждый модуль — это скилл, каждая библиотека — кольцо силы, которое нужно обуздать. Кто смог соединить их гармонией — поднимался выше, становясь Архитектором.
...
hero.status()
```
⚡ Каждая функция здесь — это медитация кода, каждое кольцо-библиотека даёт новые возможности, а слабые — сгорают, оставленные на дне ада редактора Notepad.

Настоящий программист знает: путь долог, но самосовершенствование в коде — это единственный способ достичь бессмертия Архитектора.
...
Предыдущий вариант как привелегии не юзабельны, по причине что модульность может не прижиться: нашим восприятием мы не запомним нагромождение сотни модулей на одной линии. А реактивность нужна. Поэтому контейнер будет задавать предпочтения (приманка), которые будет вмещать. Корень программы это так же контейнер. Нужно выстроить предпочтения так, чтобы это были трамплинами, из любой точки восприятия.


<h3>Линейное программирование</h3>

Отбросим ООП. Сделаем все модульным - плодим их код сколько угодно.

![](./Картинки/Линейное_программирование.jpg)

На картинке три презентации. Первая показывает всю работу с кодом, по каждому элементу можно кликнуть для редактирования, или добавить. 

На второй презентации справа настройка редактора будет в виде чекбоксов: "свернуть код до модулей", "скрыть описание элементов".

На третьей снизу презентации мы можем работать с кодом в виде отвлетлений как на git. Кликаем на чебокс по какой метке будет изменять код и у кода будет стоять особая метка, по которой мы можем быстро найти места изменения по задачи метки. И например если нужно внести срочный баг-фикс, мы скрываем изменения с определенной меткой, вносим баг-фикс, и актуализируем предыдущую работу.

Ну и чтобы не изменять по отдельности одинаковые модули - супер приз в виде синхроного изменения.

Сами модули будут изначально в отдельном окошке, их можно подключить в это окошко из внешнего источника или сделать самолично из куска кода. У каждого модуля в отдельном окошке можно посмотреть сколько раз применялось и где, плюс изменения. Там же можно их засинхронить. С окошка естественно можно перетащить модуль в код, где с модулем подтянутся внешние переменные.

Собственно переменную можно получить из модуля в локальную переменную с помощью функционала, а доставка данных в модуль будет в виде переменной с пометкой что завязан на модуль (в модуле появится призрак завязанной переменной, с которым можно взаимодействовать как с локальной переменной).

Добавляя функционал выполнения функции, будут подключены не только переменные, но и связка с этой функцией.

Условия if else в виде связки "подключить модуль если".


<h3>🐉 Боевой стиль: «Память Массива» (PHP)</h3>
