# Скил оптимальности: программирование
...
1. developer - сборочная версия на локалке
2. editor - редактируемый код по новым правилам
3. release - сборочная версия для презентации
4. production - сборочная версия как рабочий вариант

`Developer` версию собирается по факту работы `Editor`. На `Release` можно отправить определённые конструкции (их изменение). На `Production` протолкнуть то, что было протолкнуто в `Release`.

При этом, при разработке:
- в любой конструкции очередность запуска процессов первичней перед конструкциями
- ресурсам можно добавить описание, и при установке значения логикой, выводить вместо названия

Уровни доступа и права доступа теперь переходят в такие области (которые управляемы/иницируемы процессами):
1. $_GET
2. $_POST
3. $_SERVER
4. $_COOKIE
...
<img src="./Картинки/Стек%20работы.jpg" width="600" />
